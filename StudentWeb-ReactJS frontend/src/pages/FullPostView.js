  ...
  function handleCommentSubmit(event) {
    ...
    AddNewCommentToPostFunc(comment, userState.data.username, postid)
      .then((response) => {
        ...
        toast.success("Komentar je uspešno objavljen!");
      })
      .catch((error) => {
        toast.error(error);
      });
          ...
          {post && (
            <>
              <CardBody>
                ...
                <h2>Naslov : {post.title}</h2>
                <h6>Kategorija : {post.category?.name}</h6>
                ...
                <Container>
                  {userState.loggedIn && (
                    <Form onSubmit={handleCommentSubmit}>
                      ...
                      <Button  size="sm"  style={{ backgroundColor: "rgb(125, 249, 255)" }}> 
                        Dodaj komentar
                      </Button>
                    </Form>
                  )}
